...
void PProgressBar::OnDraw() {
	...
	if (_percentage > 0) {
		...
		solidrectangle(_rectangle.left + 3, _rectangle.top + 3, _rectangle.left + 3 + static_cast<LONG>(delta), _rectangle.bottom - 4);
	}
}
void PProgressBar::SetPercentage(const float& Percentage) {
	...
	if (_percentage == 100) OnFinished((int)_percentage);

	OnPercentageChanged(Percentage);
}
int PProgressBar::GetPercentage() const {
	return static_cast<int>(_percentage);
}
